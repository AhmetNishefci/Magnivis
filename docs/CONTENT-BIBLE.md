# Content bible

## Viewer contract

Every video should leave the viewer with a clearer mental model and an earned feeling of awe. A strong Magnivis story repeatedly reframes the viewer's reference point.

## Story standards

- Hook with a visual or statement the viewer understands immediately.
- Establish one reference object or quantity before escalating.
- Make comparisons spatially honest; label approximations and uncertainty.
- Use few words, concrete phrasing, and one idea per text beat.
- Let scale drive transitions. Prefer visual proof over narration-heavy explanation.
- End quickly on the largest conceptual reframe. No retention-killing outro.

## Accuracy standards

- Distinguish radius, diameter, mass, volume, distance, and apparent size.
- Do not use “largest known” unless the current evidence and uncertainty justify it.
- Avoid fake precision. Round to the level the source and story support.
- Keep citations and research notes in committed structured data even when sources do not appear on-screen.

## Shorts composition

Design for 9:16 and mobile viewing. Important text stays inside the project safe-area tokens: clear of the right interaction rail, lower metadata/caption area, and extreme top edge. Check representative frames and a real device before upload.

## Audio

Music, ambience, transitions, impacts, narration, and silence are separate conceptual layers. Sound should clarify scale and pacing. Narration may be added later without rebuilding visual scenes.

